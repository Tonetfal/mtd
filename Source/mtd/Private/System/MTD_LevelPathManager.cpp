...
void AMTD_LevelPathManager::BeginPlay()
{
    ...
    for (const FMTD_Path &Path :Paths)
    {
        ...
        {
            MTD_WARN("Path '%s' is empty, so it will be removed from the list!", *Path.Name.ToString());
            PathsToDelete.Add(i);
        }
        i++;
    }
...
TArray<FVector> AMTD_LevelPathManager::GetNavigationPath(FVector Location, FName PathChannel)
{
