...
void UMTD_AbilitySystemComponent::ProcessAbilityInput(
	float DeltaSeconds, bool bGamePaused)
{
	...
	for (const FGameplayAbilitySpecHandle &SpecHandle : InputPressedSpecHandles)
	{
		...
		{
			const auto Ability =
				CastChecked<UMTD_GameplayAbility>(Spec->Ability);
			check(Ability);
...
void UMTD_AbilitySystemComponent::OnAbilityInputTagPressed(
...
void UMTD_AbilitySystemComponent::OnAbilityInputTagReleased(
	const FGameplayTag &InputTag)
{
	if (!InputTag.IsValid())
		return;
