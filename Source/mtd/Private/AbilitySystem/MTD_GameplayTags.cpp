...
void FMTD_GameplayTags::AddMovementModeTag(
...
void FMTD_GameplayTags::AddCustomMovementModeTag(
	FGameplayTag &OutTag, const ANSICHAR *TagName, uint8 CustomMovementMode)
{
...
FGameplayTag FMTD_GameplayTags::FindTagByString(
	FString TagString, bool bMatchPartialString)
{
	...
	if (!Tag.IsValid() && bMatchPartialString)
	{
		...
		for (const FGameplayTag TestTag : AllTags)
		{
			...
			{
				MTD_LOG("Could not find exact match for tag [%s] but found "
					"partial match on tag [%s].", *TagString,
					*TestTag.ToString());
				Tag = TestTag;
				break;
			}
