    ...
    TSubclassOf<UMTD_EquipmentDefinition> EquipmentDefinition)
{
    ...
    const auto Equipment = GetDefault<UMTD_EquipmentDefinition>(EquipmentDefinition);

    auto MtdAsc = Cast<UMTD_AbilitySystemComponent>(UAbilitySystemGlobals::GetAbilitySystemComponentFromActor(Owner));
