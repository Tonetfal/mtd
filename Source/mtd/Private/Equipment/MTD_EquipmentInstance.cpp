...
APawn *UMTD_EquipmentInstance::GetTypedPawn(TSubclassOf<APawn> PawnType) const
{
    ...
    return ((PawnType) && (IsValid(Outer)) && (Outer->IsA(PawnType))) ? (Cast<APawn>(Outer)) : (nullptr);
}

UAbilitySystemComponent *UMTD_EquipmentInstance::GetAbilitySystemComponent() const
{
    const APawn *Pawn = GetPawn();
    return (IsValid(Pawn)) ? (UAbilitySystemGlobals::GetAbilitySystemComponentFromActor(Pawn)) : (nullptr);
}
...
void UMTD_EquipmentInstance::SpawnEquipmentActor(const FMTD_EquipmentActorToSpawn &ActorToSpawn)
{
    APawn *OwningPawn = GetPawn();
    ...
    auto Character = Cast<ACharacter>(OwningPawn);
    if (IsValid(Character))
    {
        AttachTarget = Character->GetMesh();
    }

    auto NewActor = GetWorld()->SpawnActorDeferred<AActor>(ActorToSpawn.ActorToSpawn, FTransform::Identity, OwningPawn);
...
void UMTD_EquipmentInstance::ModStats(float Multiplier)
{
    ...
    if (!IsValid(Asc))
    {
        MTDS_ERROR("Cannot modify stats to owner [%s] with a NULL ability system", *GetNameSafe(GetOuter()));
        return;
    }
