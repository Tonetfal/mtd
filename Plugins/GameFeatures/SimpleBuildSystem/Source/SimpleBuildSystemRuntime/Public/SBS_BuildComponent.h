...
public:
	...
	/// The static mesh used by the ghost building actor. Its collision
	/// determines whether the final object will be placed at the location the
	/// player is willing to place it at.
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite)
	...
	/// The material used by the ghost building actor when it's allowed to
	/// build.
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite)
...
public:
	...
	/// Enable build mode by initializing everything related and giving the new
	/// inputs.
	UFUNCTION(BlueprintCallable, Category="Build Component")
	bool BuildModeEnable(FSBS_BuildingData Data);
	
	/// Forget about old building data, use the new ones. Reset the building
	/// state to the default one.
	UFUNCTION(BlueprintCallable, Category="Build Component")
	...
	/// Procede to the next building stage. For example, moving, rotating,
	/// building.
	UFUNCTION(BlueprintCallable, Category="Build Component")
	void BuildConfirm();
...
protected:
	...
	/// Called after BuildFinish. Should be overrode with custom functionality.
	/// Disables Build Mode by default.
	UFUNCTION(BlueprintNativeEvent, Category="Build Component")
	void BuildStop();
	virtual void BuildStop_Implementation();

	/// Called when Build Progress finishes. Spawns and setups the building
	/// object.
	...
	/// Called each tick when Building State is Building. Builds up the
	/// progress, and calls BuildFinish on finish.
	UFUNCTION(BlueprintNativeEvent, Category="Build Component")
	...
	virtual float GetBuildProgressRatioPerTick_Implementation(
		float DeltaSeconds) const;

	/// Adds some build progress and notifies about it using
	/// OnBuildProgressChangedDelegate.
	void AddBuildProgressRatio(float ProgressRatio);

	/// Sets a new state and notifies about it using
	/// OnBuildStateChangedDelegate.
	UFUNCTION(BlueprintCallable, Category="Build Component")
...
private:
	...
	/// Input mapping context that will be active only when the build mode is
	/// active as well.
	UPROPERTY(EditDefaultsOnly, Category="Build Component")
	...
	/// Length of the trace done from the camera location following its forward
	/// vector to find any surface.
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category="Build Component",
		meta=(AllowPrivateAccess="true", ClampMin="1.0"))
	float MoveTraceLength = 1000.f;

	/// Length of the trace done downwards when placing an object to find a
	/// horizontal surface.
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category="Build Component",
		meta=(AllowPrivateAccess="true", ClampMin="1.0"))
	...
	/// Length of the trace done from the camera location following its forward
	/// vector to find a surface the ghost build object has to face.
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category="Build Component",
		meta=(AllowPrivateAccess="true", ClampMin="1.0"))
	float RotateTraceLength = 1000.f;

	/// Object types that the building object can be tried to be placed on, and
	/// rotated towards.
	UPROPERTY(EditDefaultsOnly, Category="Build Component")
