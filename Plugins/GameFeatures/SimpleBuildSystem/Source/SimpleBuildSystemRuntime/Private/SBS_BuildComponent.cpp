...
bool FSBS_BuildingData::IsValid() const
{
	return
		ObjectClass &&
		::IsValid(StaticMesh) &&
		::IsValid(AllowMaterial) &&
		::IsValid(ForbidMaterial);
}
...
USBS_BuildComponent::USBS_BuildComponent(
...
void USBS_BuildComponent::MoveBuildGhostActor_Implementation()
{
	...
	// The object should be above the ground to make further traces work
	// properly regardless the floating point imprecision
...
void USBS_BuildComponent::RotateBuildGhostActor_Implementation()
{
	...
	// Rotate by Y
	BuildGhostActor->SetActorRotation(
		FRotator(0.f, AngleDeg, 0.f), ETeleportType::TeleportPhysics);
}
	...
	FActorComponentTickFunction *ThisTickFunction)
{
	...
	switch (BuildState)
	{
	case ESBS_BuildState::Moving: MoveBuildGhostActor(); break;
	case ESBS_BuildState::Rotating: RotateBuildGhostActor(); break;
	case ESBS_BuildState::Building: ProgressBuilding(DeltaSeconds); break;
	case ESBS_BuildState::Finished: BuildFinish(); BuildStop(); break;
	default: break;
	}
}
